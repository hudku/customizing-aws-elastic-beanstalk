...
#
# Given Beanstalk environment name obtain the end point URL (URL of the Load Balancer).
#
...
elbURL=$(elastic-beanstalk-describe-environments -e $envName -j | python -mjson.tool | grep EndpointURL | awk '{print $2}' | sed s/\"//g | sed s/,//g)
